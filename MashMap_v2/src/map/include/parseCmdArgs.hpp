...
namespace skch
{
    ...
    void initCmdParser(CommandLineProcessing::ArgvParser &cmd)
    {
        ...
        cmd.defineOption("gfa", "gfa file.", ArgvParser::OptionRequiresValue);
        cmd.defineOptionAlternative("gfa", "g");
    }
            ...
            skch::Parameters &parameters)
    {
        ...
        if(cmd.foundOption("gfa")){
            std::string gfaFile;

            str << cmd.optionValue("gfa");
            str >> gfaFile;

            parameters.gfaSequences.push_back(gfaFile);
        }
        else{
        }
